...
QTTT.BoardModels.Simple.BoardGraph = {
	...
	var obj = {
		...
		if (this._started){
		    ...
		    this._nm.set(this._prev_node.id, this._current_node.id, this._fragment.move_number);
		}
		...
		resolution.each(function(index, move_fragment){
		    ...
		    for (var i = 0; i<move_fragment.field;i++) nm.set(i, move_fragment.field,0);
		    nm.set(move_fragment.field, move_fragment.field, move_fragment.move_number);
		});
		this._restart();
	    },
	    _restart: function(){
	    ...
	    reset: function(){
		this._nm = QTTT.BoardModels.NeighborhoodMatrix.new();
		this._counter.init();
		this._graph.reset();
	    },
